...
pub mod coin_flip_solana {
    ...
    pub fn end_coin_flip(ctx: Context<EndCoinFlip>, wager: u64) -> Result<()> {
        let coin_flip = &mut ctx.accounts.coin_flip;

        require!(coin_flip.is_active, ErrorCode::GameAlreadyFinished);

        let starting_wager = coin_flip.starting_wager;
        let min_wager = starting_wager
            .checked_mul(99)
            .unwrap()
            .checked_div(100)
            .unwrap();
        let max_wager = starting_wager
            .checked_mul(101)
            .unwrap()
            .checked_div(100)
            .unwrap();
...
mod tests {
    ...
    fn test_random_result_range() {
        ...
        let random_seed = timestamp.wrapping_add(slot).wrapping_add(player_byte);
